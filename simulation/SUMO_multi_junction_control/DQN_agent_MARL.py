    ...
    def __init__(self, args, env):
        ...
        for i in range(self.num_junctions):
            if self.args.nbobs == 'close':
                self.policy_net.append(DQN_Linear(self.n_obs * 5, self.n_actions))
                self.target_net.append(DQN_Linear(self.n_obs * 5, self.n_actions))
            elif self.args.nbobs == 'far':
                self.policy_net.append(DQN_Linear(self.n_obs * 13, self.n_actions))
                self.target_net.append(DQN_Linear(self.n_obs * 13, self.n_actions))
            else:
    ...
    def learn(self):
        ...
        for episode in range(self.args.num_episodes):
            ...
            for t in range(self.args.episode_length):
                ...
                if self.args.nbobs != 'None':
                    obs_new = self.obs_transfer_add_nbs(obs_new)
            ...
            np.save(self.save_path + '/MARL_farnbs_episode_return_queue_reward_flowprob0.05.npy', episode_return_all)
            np.save(self.save_path + '/MARL_farnbs_avg_traffic_load_queue_reward_flowprob0.05.npy', episode_avg_traffic_load_all)

            for i in range(self.num_junctions):
                torch.save(self.policy_net[i].state_dict(), self.save_path + '/MARL_models/MARL_farnbs_policy_net_junction' + str(i) + '_flowprob0.05.pt')
...
if __name__ == '__main__':
    ...
    trainer = DQN_agent_MARL(args, env)
