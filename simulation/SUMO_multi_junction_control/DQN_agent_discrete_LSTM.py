...
class LSTM(nn.Module):
    ...
    def __init__(self, n_obs, hidden_size, n_actions):
        ...
        self.lstm = nn.LSTM(n_obs, hidden_size, batch_first=True)
...
class DQN_agent_discrete_LSTM:
    def __init__(self, args, env):
        ...
        self.hidden_h = torch.zeros([self.num_junctions,1,1,self.hidden_size]).to(self.device)
        self.hidden_c = torch.zeros([self.num_junctions,1,1,self.hidden_size]).to(self.device)
        ...
        for i in range(self.num_junctions):
            self.policy_net.append(LSTM(self.n_obs, self.hidden_size, self.n_actions).to(self.device))
            self.target_net.append(LSTM(self.n_obs, self.hidden_size, self.n_actions).to(self.device))
    ...
    def learn(self):
        ...
        for episode in range(self.args.num_episodes):
            ...
            np.save(self.save_path + '/DiscreteRL_LSTM_episode_return_queue_reward_flowprob0.5.npy', episode_return_all)
            np.save(self.save_path + '/DiscreteRL_LSTM_avg_traffic_load_queue_reward_flowprob0.5.npy', episode_avg_traffic_load_all)

            for i in range(self.num_junctions):
                torch.save(self.policy_net[i].state_dict(), self.save_path + '/DiscreteRL_LSTM_models/DiscreteRL_LSTM_policy_net_junction' + str(i) + '_flowprob0.5.pt')
    ...
    def _optimize_model(self, batch_size, junction_id):
        ...
        train_hidden_h = torch.zeros([1,batch_size,self.hidden_size]).to(self.device)
        train_hidden_c = torch.zeros([1,batch_size,self.hidden_size]).to(self.device)

        q_target,_ = self.target_net[junction_id](next_observations, (train_hidden_h, train_hidden_c))
        ...
        train_hidden_h = torch.zeros([1,batch_size,self.hidden_size]).to(self.device)
        train_hidden_c = torch.zeros([1,batch_size,self.hidden_size]).to(self.device)
        q_policy,_ = self.policy_net[junction_id](observations, (train_hidden_h, train_hidden_c))
...
if __name__ == '__main__':
    ...
    trainer = DQN_agent_discrete_LSTM(args, env)
